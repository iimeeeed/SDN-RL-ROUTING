...
"""
R-Optimizer (policy-gradient) controller for topology-aware SDN routing.

Usage:
	TOPOLOGY=abilene ryu-manager controllers/roptimizer.py
	TOPOLOGY=fat_tree ryu-manager controllers/roptimizer.py
"""
...
class ROptimizerController(app_manager.RyuApp):
	OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

	def __init__(self, *args, **kwargs):
		super(ROptimizerController, self).__init__(*args, **kwargs)
	...
	def initialize_policy(self, state: PolicyState, actions: List[str]) -> None:
		...
		self.logger.info(
			"Initialized policy for state=%s actions=%s",
			state,
			len(actions)
		)
	...
	def initialize_from_q_table(self, q_table: Dict[Tuple[str, str], Dict[Tuple[str, ...], float]], bias: float = 0.7) -> None:
		seeded_states = 0
		for (src_switch, dst_switch), path_scores in q_table.items():
			if not path_scores:
				continue
			...
			for index in range(len(best_path) - 1):
				...
				if next_hop in actions:
					if len(actions) == 1:
						...
						remainder = max(1.0 - bias, 0.0)
						per_other = remainder / (len(actions) - 1)
						self.policy[state] = {
							action: (bias if action == next_hop else per_other)
		...
		self.logger.info(
			"Initialized policy from Q-table: seeded_states=%s bias=%.2f",
			seeded_states,
			bias
		)
