...
```text
mininet> py __import__('traffic.generate_traffic', fromlist=['run']).run(net)
```
...
```text
mininet> py __import__('traffic.generate_traffic', fromlist=['run']).run(net, pairs=[("h1","h5")], protocols="tcp", duration=30)
```
...
- src, dst, protocol, port, duration_s
- throughput_mbps (TCP)
